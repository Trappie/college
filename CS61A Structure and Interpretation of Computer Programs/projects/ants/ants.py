...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect.is_true: # if the insect is a queen ant and is a true one
                return
            # END Problem 13
...
class Ant(Insect):
    ...
    # if self and contain other ant, return True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0 # to store the distance from target_place to the ant
        # find the min_range place
        for _ in range(0, self.min_range):
            target_place = target_place.entrance
            distance += 1

        while target_place != hive and distance <= self.max_range:
            if target_place.bees:
                return random_or_none(target_place.bees)
            else:
                target_place = target_place.entrance
                distance += 1
        return None
        # return random_or_none(self.place.bees) # choose one enemy from the same place with the bee
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            list_copy = self.place.bees[:]
            for bee in list_copy:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.place.bees:
            tmp_bee_list = self.place.bees[:]
            for bee in tmp_bee_list:
                bee.reduce_armor(self.damage)
        # END Problem 7
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        if bee is not None:
            ...
            self.digesting_counting = self.time_to_digest
        # END Problem 6

    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.is_ready_to_eat():
            self.eat_bee(random_or_none(self.place.bees))
        else:
            self.digesting_counting -= 1

        # END Problem 6

    def is_ready_to_eat(self):
        return self.digesting_counting == 0
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if QueenAnt.is_exist:
            self.is_true = False
        else:
            self.is_true = True
            QueenAnt.is_exist = True
        self.buffed_list = []
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true:
            ...
            #todo double the damage of ants behind it
            self.buff()
        else:
    ...
    def buff(self):
        ...
        while place_pointer: # should rewrite it with data sets mode, easy to extend and modified
            if place_pointer.ant:
                ant = place_pointer.ant
                if ant.container:
                    if ant not in self.buffed_list:
                        ant.damage *= 2
                        self.buffed_list.append(ant)
                    ant = ant.ant
                if ant and ant not in self.buffed_list:
                    ant.damage *= 2
                    self.buffed_list.append(ant)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    # duration would be the number of turns that effect last
    # bee is the one whose aciton will be modified
    # effect is the function which take in the original function and return the modified function
    # when time up, the action of the bee should be restore to its original one
    "*** YOUR CODE HERE ***"
    original_action = bee.action
    modified_action = effect(original_action)
    bee.action = get_counted_modified_action(bee, original_action, modified_action, duration)

def get_counted_modified_action(bee, ori, modi, duration):
    # return a modified method that can be called as an action, but everytime it's called it will check the time left, if time up, change the action to the default one
    # so the modified action should know something: 1. the original action to restore; 2. the duration to have effect; 3. the obj itself?
    def counted_modified(colony):
        nonlocal duration
        if duration > 0:
            modi(colony)
            duration -= 1
            ...
            ori(colony)
            bee.action = ori
    return counted_modified
    # NOTIFICATION: this is the longest question I have met in python 3 for now, don't hasitate to make the logic easier by introducing help methods 
