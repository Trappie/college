...
def scheme_read(src):
    ...
    if val == 'nil':
        return nil
    # if the token is '(', means the following should be a whole expression, should return the whole expression
    elif val == '(':
        return read_tail(src)
    elif val == "'":
        # BEGIN PROBLEM 7
        "*** YOUR CODE HERE ***"
        # put everything in the following src into a well-formed pair
        return Pair('quote', Pair(scheme_read(src), nil))
        # END PROBLEM 7
    # delimiters contains . , ,@    what is the last one?
    # if token not in the range, return the token directly
...
def read_tail(src):
    ...
    Pair(1, 2)
    """
    # current should not be none
    # if the current is ), means the reading finished, return nil to cons a full pair
    # if the current is . then it's a dot syntax
    # else means you should 
    try:
        if src.current() is None:
            ...
            if src.current() == ')':
                ...
                raise SyntaxError('unexpected second element after dot {0}'.format(src.current()))
            # END PROBLEM 2
        else:
            # build the first of the pair
            first = scheme_read(src)
            # build the rest of the pair by call it self
            rest = read_tail(src)
