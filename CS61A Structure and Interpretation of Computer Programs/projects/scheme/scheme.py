...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS: # if first is symbol and in special forms
        return SPECIAL_FORMS[first](rest, env)
    else:
        # BEGIN PROBLEM 5
        operator = scheme_eval(expr.first, env)
        # for lambda procedures, the operator will be the expression of lambda
        # if it's not a procedure, the following line will raise an exception
        check_procedure(operator)
        # before evaluable the current one, evaluate all sub expressions first recursively
        # map is an instance method of Pair to map the given method to every thing in the pair-shap list
        operands = expr.second.map(lambda ex: scheme_eval(ex, env))
        # return the result of the applying
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        rest = eval_all(expressions.second, env)
        if rest != None:
            return rest
        else:
            return curr
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("the number of values and formal parameters don't match")
        while formals != nil:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong argument')
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def __init__(self, formals, body, env):
        ...
        # scheme list of the formal parameters that names the arguments
        self.formals = formals
        # scheme list of expressions
        self.body = body
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # evalueate the second's content
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    # check each one in the expression, if it's false, return #f 
    last = True
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        check_form(clause, 1) # there should be at least one sub expression
        ...
        if scheme_truep(test):
            if clause.second != nil:
                # evaluate all the content after the true one and return the last one
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        if not scheme_symbolp(pair.first):
            raise SchemeError('wrong form for let')
        formals = Pair(pair.first, formals)
        vals = Pair(scheme_eval(pair.second.first, env), vals)
        bindings = bindings.second
    return env.make_child_frame(formals, vals)
    # (let ((x 5) (y x)) (expression to return)) so this expression will let x = 5 and y = x
    # END PROBLEM 15
...
def do_mu_form(expressions, env): # the env here is the call environment
    """Evaluate a mu form."""
    check_form(expressions, 2)
    formals = expressions.first
    check_formals(formals)
    # BEGIN PROBLEM 16
    # to return a mu_procedure, you need formals and bodys
    body = expressions.second
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        """
        # if the incoming expression is tail and it's not done yet(need more evaluation), then return the place holder object Thunk
        
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            return Thunk(expr, env)
        # else, the result is the final answer 
        else:
