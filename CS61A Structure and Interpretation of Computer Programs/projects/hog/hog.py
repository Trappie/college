...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if who:
            curr_score = score1
        new_high = 0
        diff = curr_score - previous_score
        if diff > previous_high:
            if diff == 1:
                print("1 point! That's the biggest gain yet for Player " + str(who))
            else:
                print (str(diff) + " points! That's the biggest gain yet for Player " + str(who))
            new_high = diff
        else:
            new_high = previous_high
        return announce_highest(who, new_high, curr_score)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    "*** YOUR CODE HERE ***"
    roll, highest = 0, 0
    test_func = make_averaged(roll_dice, num_samples)
    for i in range(1, 11):
        average_score = test_func(i, dice)
        if average_score > highest:
            roll = i
            highest = average_score
    return roll
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    zero_output = free_bacon(opponent_score)
    new_score = zero_output + score
    if new_score < opponent_score and is_swap(new_score, opponent_score):
        return 0
