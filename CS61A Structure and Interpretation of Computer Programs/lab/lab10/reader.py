...
# find all the sequences chars in the allowed_characters and return them as a whole
...
def next_token(src):
    ...
    if c is None:
        return None
    elif c in NUMERAL: # if token is a number
        literal = take(src, NUMERAL) # get the whole body of the number
        try:
            return int(literal) # return it as int if possible
        except ValueError:
            try:
                return float(literal) # if not int, return as float
            except ValueError:
