...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        raise NameError("Can't find {0} name in the environment {1}".format(self.string, env))
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        # evaluate the operator and then apply it to all operands come from evaluating expressions
        func = self.operator.eval(env)
        return func.apply([expr.eval(env) for expr in self.operands])
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        # make a copy of parent environment and modify some of the variables to make a new environment which makes it possible to search up efficiently
        current_env = self.parent.copy()
        for i in range(0, len(self.parameters)):
            current_env[self.parameters[i]] = arguments[i]
